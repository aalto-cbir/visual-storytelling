...
def train(opt):
    ...
    elif os.path.exists(os.path.join(logger.log_dir, 'disc-model.pth')):
        logging.info("loading pretrained RewardModel")
        disc.load_state_dict(torch.load(os.path.join(logger.log_dir, 'disc-model.pth')))
    disc.cuda()

    # set up optimizer
    optimizer = setup_optimizer(opt, model, optim_path)
    disc_optimizer = setup_optimizer(opt, disc, disc_optim_path)
    ...
    for epoch in range(logger.epoch_start, opt.max_epochs):
        ...
        for iter, batch in enumerate(train_loader):
            ...
            if flag.flag == "Disc":
                ...
                loss = torch.sum(gen_score)
                # if logger.iteration % opt.losses_log_every == 0:
                avg_pos_score = torch.mean(gen_score)
                logging.info(
                    "policy loss: {} average IRL score: {}".format(loss.data[0], avg_pos_score.data[0]))
