...
def setup_optimizer(opt, model, resume_from_optim_path=None):
    if opt.optim == 'Adam':
    ...
    # Load the optimizer
    if resume_from_optim_path is not None and os.path.isfile(resume_from_optim_path):
        logging.info("Load optimizer from {}".format(resume_from_optim_path))
        optimizer.load_state_dict(torch.load(resume_from_optim_path))
