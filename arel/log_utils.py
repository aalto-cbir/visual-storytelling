...
class Logger:
    ...
    def log_checkpoint(self, epoch, val_loss, metrics, predictions, opt, model, dataset, optimizer=None,
                       disc_model=None, disc_optimizer=None):
        ...
        if disc_model is not None:
            disc_model_path = os.path.join(self.log_dir, 'disc-model.pth')
            torch.save(disc_model.state_dict(), disc_model_path)
            logging.info('disc model saved to {}'.format(disc_model_path))

        # save disc optimizer
        if disc_optimizer is not None:
            disc_optimizer_path = os.path.join(self.log_dir, 'disc-optimizer.pth')
            torch.save(disc_optimizer.state_dict(), disc_optimizer_path)
