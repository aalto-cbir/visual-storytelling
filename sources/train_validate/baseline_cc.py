...
from sources.models.baseline import Encoder, SemanticDecoder
...
class BaselineCC:
    ...
    def _load_data(self, args):
        ...
        with open(args.semantics_loc, 'rb') as file:
            self.semantics = pickle.load(file)
        file.close()

        print(f'Loaded {len(self.semantics)} semantics dictionary.')

    def _build_model(self, args):
        self.encoder = Encoder(self.params)
        self.decoder = SemanticDecoder(self.params, len(self.vocab))
        ...
        self.criterion = torch.nn.CrossEntropyLoss(reduce=False, reduction='none')
    ...
    def train(self, epoch, epochs, log_step):
        ...
        for idx, (sequences, target_stories, target_lengths, story_ids, feature_sets) in enumerate(
            ...
            sequence_data = BaselineCC.torchify_sequence(sequences)
            context_vector, sequence_features = self.encoder(sequence_data)
            semantics_batch = torch.stack([torch.from_numpy(self.semantics[story_ids[idx]])
                                           for idx in range(len(story_ids))])
            target_stories = target_stories.to(device)
            target_lengths = torch.from_numpy(np.asarray(target_lengths)).to(device)
            targets = pack_padded_sequence(target_stories, target_lengths, batch_first=True)[0]
            outputs = self.decoder(context_vector.squeeze(0), sequence_features, semantics_batch.to(device),
                                   target_stories, target_lengths)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_basename', type=str, default='baseline_cc',
    ...
    parser.add_argument('--semantics_size', type=int, default=1470,
                        help='dimension of semantic character vectors')
    ...
    parser.add_argument('--semantics_loc', type=str, default='../../resources/story_id_2_semantics.pkl')
