...
def save_it(args):
    ...
    all_stories_train = read_pickle_dict('../../resources/story_id_2_story_train.pkl')
    all_stories_val = read_pickle_dict('../../resources/story_id_2_story_val.pkl')
    all_stories_test = read_pickle_dict('../../resources/story_id_2_story_test.pkl')
    ...
    train = [list(all_stories_train.values()), range(len(all_stories_train.keys())), list(all_stories_train.keys())]
    val = [list(all_stories_val.values()), range(len(all_stories_val.keys())), list(all_stories_val.keys())]
    test = [list(all_stories_test.values()), range(len(all_stories_test.keys())), list(all_stories_test.keys())]
...
def sequences_and_stories(args):
    ...
    for batch_idx, (sequences, targets1, targets2, targets3, targets4, targets5, story_ids, feature_sets) in enumerate(
        ...
        for idx in range(cur_batch_size):
            ...
            cur_story = []
            cur_story.extend(list(targets1[idx][1].numpy()))
            cur_story.extend(list(targets2[idx][1].numpy()))
            cur_story.extend(list(targets3[idx][1].numpy()))
            cur_story.extend(list(targets4[idx][1].numpy()))
            cur_story.extend(list(targets5[idx][1].numpy()))
            story_id_2_story[cur_story_id] = cur_story

        print(f'{batch_idx}/{len(data_loader)} batches complete!')

    print('saving as pkls temporarily...\n')
    with open(f'../../resources/story_id_2_story_{args.datasplit}.pkl', 'wb') as file:
        pickle.dump(story_id_2_story, file)
    file.close()
